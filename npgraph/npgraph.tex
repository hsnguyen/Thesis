...
\caption{Comparison of assemblies produced in batch-mode by \npgraph{} and the comparative methods} \label{table:npgraph_compare} \\
...
For \emph{Acinetobacter} AB30 synthetic data, it was an deficiency for \npscarf{}\_wag in traversing the graph to find candidate paths for a bridge of long distance due to particular large search space.
...
Due to the lack of available references, lesser statistics were reported by QUAST for the comparison of the results. 
...
From the first data set, there were high similarity between final contigs generated by two assemblers.
They shared the same number of circular ultimate sequences, including the chromosomal and other six replicons contigs. 
In which, the only divergence lied on the biggest sequence ($\simeq 5.029$Mbp) when the \unicycler{}'s chromosome was 48 nucleotides longer than that of \npgraph{}.
...
Lastly, there were still $14.5$Kbp unfinished sequences, being fragmented as 3 contigs in \unicycler{} and 2 for \npscarf{}.

The assembly task for \emph{Enterobacter~cloacae} was more challenging as the chromosomal DNA sequence had not been fully resolved using both methods. 
...
Similar to the previous data set, there were around $14$Kpb of data were unable to finish by the assemblers.
