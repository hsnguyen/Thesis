...
{--Sam Kean}
% This chapter will provide background and comparison between sequencing technologies, from NGS (Illumina as example) to PacBio SMRT and Oxford Nanopore Technology sequencing methodology. The focus on ONT nanopore sequencing will be described later via its pros and cons. I will review its forte by key features such as portability and real-time in general through several representative achievements to date. After that I will depict some of the challenges in the field of bioinformatics on using nanopore data for analysis.

% Next, I will review the works related to my thesis by showing how long reads has been used to complete the fragmented genomes and the meaning of doing so in real-time. 
% At first, the history of genome assembly will be briefly described, concentrating on different approaches in methodology. Secondly, I will summarize the current tools used for genome assembly that utilize nanopore long reads.  For the next part, it will come across streaming algorithms for general applications and for nanopore data analysis in particular. Finally, the chapter will be finished by the motivation of using such data to finish genomes in real-time. 

% This chapter will end outlining the rest of the thesis content.
\clearpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% due to its critical engagement in life science.  %%LC this bit did not make sense to me
...
%Although it does not directly mean ``decoding life" since there would be a huge amount of additional works to interpret the output data and unravel biological %pathways, this task sets the very first background to establish such knowledge.
% * <hoangnguyen177@gmail.com> 2018-12-03T02:00:37.908Z:
%
% ^.
...
%%In case it is the complete set of DNA sequences from an organism's genome being sequenced, we have a Whole Genome Sequencing (WGS) run.
...
The cost of sequencing has been dropped drastically over time to the point that scientists are now planning for the Earth BioGenome Project, which plans to sequence 1.5 million different species across multiple continents\cite{Lewin2018earth}. %, but with a smaller cost compared to the very first human genome project
...
In fact, Sanger sequencing contributed vastly at the beginning of the sequencing era but became supplanted later by \emph{Next Generation Sequencing} (NGS) machines which are of much larger scale and higher throughput in term of data acquired. For that reason, it is restricted in use today, and used largely only for validation or in combination with other deep-sequencing methods \cite{Freeman2009}.
NGS platforms, on the other hand, can output a large amount of DNA sequence with length up to 1 kbp in form of single, mate-paired or paired-end reads on demand, for a large ranges of studies.
...
In fact, the sequencing productivity, quality and deployment cost-effectiveness have been enhanced significantly with the introduction of new platforms such as MiniSeq, MiSeq series, NextSeq, HiSeq and HiSeq X and Novaseq series. These sequencers provide high-fidelity paired-end reads of length 100-300bp each.  Bacterial DNA sequencing is typically done at average sequencing depth of more than 100-fold. %%LC - you havent really discussed sequence depth 
%Nowadays the terms short-read data and Illumina data are used interchangeably in many cases.
In this thesis, data from Illumina MiSeq or HiSeq were used as input for the assembly algorithms. 

%Third genertion sequencing intro...
...
Before discussing in details these long-read sequencers, I would like to briefly review another important step after the sequencing process to fully reconstruct the genome content, namely genome assembly.  %%LC - I think assembly should probably come later? 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Genome assembly for NGS data}
\subsubsection{Definition}
\begin{figure}[ht!]
\centering
\includegraphics[width=.9\textwidth]{images/genome_decoding.jpg}
...
Before sequencing, the extracted DNA sample %%LC - first fragmeneted right?
 is normally subjected to a cloning step, usually Polymerase Chain Reaction or PCR, that generates a number of identical copies of DNA fragments with low error rate
 \cite{Garibyan2013research}. The reason is to reduce the chance of missing pieces and also to cover the inevitable errors caused during library preparation and sequencing process. 
Those pieces are then glued back together in a process called \emph{assembly} to have a draft genome before being subjected to additional post-processing step for the final complete reconstruction.  %%LC - you should spell out what this process is in more detail
...
\subsubsection{General working principle for genome assembly}
...
\subsubsection{Overview of assembly algorithms}
 
The demand for robust assemblers became overly compelling when high-throughput sequencing platforms were employed and started generating massive amount of data in the age of NGS.
The abundance of randomly distributed short reads from NGS platforms had motivated substantial research in bioinformatics toward solving the genome assembly problem in an efficient way\cite{Liu2012}. 
...
\paragraph{De Bruijin Graph} To construct the graph, reads are broken into consecutive \emph{k-mers} for the set of vertices, whilst edges show adjacencies between \emph{k-mers} with $k-1$ overlap. The data structure is actually storing fixed-length words and their linkages, making its memory requirement dependent only on the genome size but not data size. The graph is then targeted to errors removal based on \emph{k-mers} spectrum, as well as trimming and simplification in which non-branched paths are merged together.
At this point, the assembly problem can be reformulated as finding an Eulerian path which defined as a walk visiting each all edges on the ultimate graph %%LC comment - why ultimate graph? 
 exactly once. 
The complexity of this algorithm is dependent on properties of the \emph{k-mers} rather than the number of reads. With a smaller $k$, the more details of overlaps emerge in the graph, but also the search space becomes bigger, not to mention the effect of repetitive elements.  
%%LC comment - you should say what is the effect on repeats, rather than say  not to mention the effect
...
On the other hand, DBG approach first breaks reads into set of consecutive \emph{k-mer}s ($k=5$ in this case). Unlike OLC, no pairwise alignments need to be explicitly carried out. The rationale is that if there exists a decent hash function, every \emph{k-mer} would be assigned by an unique value so that same words are joined together thus as the consequence, every possible overlapping information are covered in the graph. %%LC comment - this is the first time you mention a hash function, but without explanation. Also what is a 'decent' hash function, you dont describe it.  Also, is it relevant here? Does the DBG explicitly use a hash function?  I thought that was more commonly used in OLC to make the overlap step more efficient.  
As the consequence, the DBG graph is more efficient in term of construction, however, would be more challenging to solve compared to the reduced OLC counterpart since the integrity of reads are usually lost after being broken into smaller pieces. 
At the same time, the repeat nature of original sequence would create branches from nodes, 
%%LC comment - you mean would split the OLC nodes into lots of branches? 
 making the graph  more fragmented and difficult to traverse.

As mentioned earlier, genome assembly from NGS data usually suffers from the presence of repeats, leading to generation of highly fragmented draft genomes (also known as pre-assemblies). This limits identification of structural variation, or identification of the configuration and location   of mobile genetic elements.
One theoretical solution is to have reads that exceed the `golden threshold' to span over almost different types of repeat in prokaryotes \cite{KorenP2015}, thus able to connect fragmented contigs and render continuous assemblies out of them.  %LC comment - what is meant by golden threshold? 
In fact resolving repeats has remained a bottle-neck until the recent emergence of the so-called Third Generation Sequencing technologies with the introduction of two instruments PacBio RS and Oxford Nanopore MinION that were specially designed for long-read sequencing. Thanks to these sequencers, DNA molecules with continuous length of up to tens of kilo base-pairs or even more could be sequenced in full.
 As a result, long-read data have been widely exploited to finish genomes \cite{BashirKR2013,KarlssonLS2015} or identify mobile genetic elements of interest \cite{HudsonBM2014,AshtonND2015}, despite having a higher basecalling error rate.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Third-generation sequencing technology}
\subsection{Single molecule, real-time (SMRT) sequencing}
...
%%Throughout perpetually developing, %%LC comment this does not make sense
 Both the read length and accuracy of the instrument have been continuing to increase significantly. The average accuracy per read has increased from about 82\% in the first release to 87\% \cite{Eid2009,Koren2013} whilst the maximum read length has reached over 50kb in recent runs \cite{Berlin2015}. 
...
%We were amongst the first participants and still actively employing MinION for various applications.  %% not sure relevant to thesis
Of the particular advantages using this device is its greater portability and flexibility.
% This thumbnail device had been preferred to PacBio platforms in generating long-read data despite of inferior throughputs, thanks to its higher mobility and %cost-effectiveness.
...
%%\EG{} ultra-long reads collection. %%LC comment actually ultra long is based on rapid
...
%Moreover, ONT introduces \emph{pay-as-you-go} pricing, where users are charged on the amount of the data obtained, which will further reduce financial barriers to use its technology. %%LC comment is this relevant? 
The chemistry and computational features are likewise updated continuously to help reduce the noises from output. 

Furthermore, GridION and ProthmethION -- the scaled-up version of MinION, are made available by the company through its access program and has already been deployed for several early experiments.  
To conclude, this is a promising and fast-growing technology that could facilitate substantial applications in real-life.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Nanopore long-read data at first glance}

Together with PacBio RS series, the ONT MinION sequencing device, through its early access programme (MAP), has become a prominent long-read data generator for researchers worldwide. Despite the relatively high error rates, the nanopore sequence approach is commonly agreed to be a  turning point from NGS due to its ability to sequence much longer reads  (tens or even hundreds of kilo-basepairs) in a real-time fashion. On the other hand, the introduced sequencing data properties would change the traditional approaches used by bioinformaticians working with genomics data.  %%LC comment - not sure about this paragraph 

\subsection{Data description}

\begin{figure}[ht!]
\centering
\subfloat[R6\label{fig:r6}]{
  \includegraphics[width=.5\textwidth]{images/lambda_R6.png}
}
\subfloat[R7\label{fig:r7}]{
  \includegraphics[width=.5\textwidth]{images/lambda_R7.png}
}

\subfloat[R9.4\label{fig:r9.4}]{
  \includegraphics[width=.5\textwidth]{images/lambda_R9_4.png}
}
\subfloat[R9.5\label{fig:r9.5}]{
  \includegraphics[width=.5\textwidth]{images/lambda_R9_5.png}
}

\caption[Statistics of nanopore burn-in experiments using different flow cell versions]{Statistics of nanopore reads using different flow cell versions on the lambda phage sample, shown in the order in which they were released. For each flow cell, the blue normalized histogram is for read length and red one is for quality (Phred score).}
\label{F:nnp_stats}
\end{figure} %%LC comment - perhaps you could include the year/month when the flow cell was released. 

\subsubsection{Lambda burn-in experiments} 
Figure \ref{F:nnp_stats} shows statistics of our lambda experiments acquired by using progressively more recent version of MinION flow cells. This type of practice is a standard control `burn-in' experiment recommended by ONT to help users familiarize themselves with the protocol, the equipment and the result long-read data. The sample is a virus named lambda phage with small genome size of 50K base pairs. All the test experiments were run for 6 hours, except R9.5 with a 1-hour run so the sequencing yields are not included in the comparison and the histogram frequencies are normalized by the number of reads for each attempt. Even though the `burn-in' experiment is not designed to exhaustively study the potential length of nanopore reads due to the limit genome size of the sample, it is easily to see from the figure a clear improvement of read length distribution by applying newer version of flow cells. The early-staged flow cells and chemistry in the kit produced fewer reads longer than 10K, as could be observed from Figure \ref{fig:r6} and \ref{fig:r7}. This length figure has become a norm in R9.4 flow cell and the longest ones even reach 20K.  %%LC comment - what do you mean by has become a norm? 
Using the newest available flow cell R9.5 - even for just a 1-hour run - gave many more longer reads, even the maximum possible 50K that cover the whole genome.  
Similarly, the fidelity of the long reads has improved with progressive releases. The average quality scores (in Phred scale) of the base-called sequences have advanced from $5.0$ ($P_{error} \approx 0.3$) in R6 and R7 to $10.0$ ($P_{error} \approx 0.1$) in R9.4 and $13.0$ ($P_{error} \approx 0.05$) in R9.5. 
...
Even so, noises will inevitably remain in nanopore reads as the result of measurement noise at nano-scale, as compared to high-precision data generated by NGS when the average Phred score is usually 30 or more ($P_{error} \leq 0.001$).
...
The introduction of long, error-prone nanopore reads has motivated novel developments in bioinformatics, as many existing approaches had been optimised for high-quality short read data.  
%Apparently, issues came out when the dropped fidelity of long reads make them challenging to be analyzed by traditional methods. 
...
This process usually require either another reference of high quality NGS reads or a large amount of long reads for self corrections.
...
Another attempt to reduce the error rate from base-called sequences is to utillize the signal-level information. Nanopolish is designed for such task. The underlying technique is to train a hidden Markov model that measures the probability of observing a certain chain of events given a particular DNA sequence. This method has been used for polishing a draft assembly using an abundance of long-read data~\cite{LomanQS2015} and has been extended to detect methylation~\cite{Simpson2017methylation}. Nevertheless, this approach is computational expensive and usually requires parallelization which is supported in its modules.

Pairwise sequence alignment is a critical operation for many genomics analyses in general and  comparative studies in particular. To adapt to the specific error characteristics of nanopore sequencing data, a number of approaches has been introduced on top of the legacy alignment methods used for short-read NGS data. 
The ubiquitous practice is still based on the previous seeds-and-extend dynamic programming technique with modifications in term of seed finding, gap extension and chaining algorithms. In addition, there would be justified settings to relax the matching criteria thus allowing more error tolerance and lengthen the alignments. Early long-read aligners have been developed and applied for PacBio SMRT data, including but not limited to BLASR~\cite{ChaissonT2012}, LASTZ~\cite{Harris2007lastz}, LAST~\cite{KiełbasaWSH2011}, BWA-MEM~\cite{Li2013} and DAligner~\cite{Meyer2014daligner}.  %LC BWA-MEM is a short read aligner, not optimised for pacbio
Recently, several other tools has been designed specifically for comparing long-read data such as GraphMap~\cite{Sovic2016graphmap} and especially minimap~\cite{Li2016} (now updated to minimap2) which significantly reduce the running time while maintaining comparative accuracy at the same time. These available solutions have been widely accepted and invoked in a flexible ways for different analysis purposes when it comes to nanopore reads.

For the multiple sequence alignment (MSA) problem, the partial order graph data structure has been proposed to cope with errors in long reads~\cite{Lee2002multiple,Lee2003generating,Grasso2004combining}. By applying this method, the authors proved the integrity of information compared to other MSA format and developed POA as an efficient tool for large alignment and consensus calling problem. The latter was then adopted in Racon~\cite{Vaser2017racon}, a commonly used 
consensus module for long uncorrected reads .

%As part of genomics analysis, genome assembly using nanopore data likewise suffered from the its error profile. The traditional DBG method cannot be applied directly to erroneous data since the number of \emph{k-mer} would be explosive and difficult to handle due to faulty words.
%On the other hand, OLC seems to fit the data better given decent aligners. However normally an exhaustive error-correction step is required for pre- and/or post-processing.

Genome assembly is another major challenge in bioinformatics, and the DBG approach which is traditionally applied to NGS data cannot be applied directly to nanopore data, since the error rate creates an artificially large number of incorrect \emph{k-mer}. 
Approaches to genome assembly which utilise nanopore reads will be discussed in the next section, including modified versions of the de Bruijn graph . 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Genome assembly with long read data}
 Nanopore data can be used on its own for \emph{de novo} genome assembly, or in conjunction with high quality NGS data in a process called hybrid assembly. 
%At present there are 2 general strategies  for genome assembly utilising nanopore data. The first one can carry out the task with only long-read data and the second approach requires additionally high quality NGS data in a process named hybrid assembly.
...
Non-hybrid assembly tools, as stated above, follow either OLC and/or an adjusted DBG approach, described below. Accordingly, most of current OLC algorithms rely on a hierarchical approach~\cite{Jayakumar2017compare}. From this perspective, seeds are first chosen as a subset of significant long reads, shorter reads are then aligned to the seeds for error-correction before invoking assembly and polishing step as usual. %%LC comment, this could be expanded - what constitutes significant
...
In addition, an efficient OLC implementation is to apply a fast overlap finding algorithm first for the assembly skeleton, then use another consensus calling or polishing tool on the draft assembly to improve the quality. This idea has been integrated in SMARTdenovo (\url{https://github.com/ruanjue/smartdenovo}) or in a pipeline that combines 2 separately developed modules: \emph{miniasm}~\cite{Li2016} as the rapid draft constructor and \emph{Racon}~\cite{Vaser2017racon} as the consensus caller. %%LC comment - is this a good time to discuss minhash and other hashing approaches, like hyperloglog


Regarding to other class, \emph{ABruijn}~\cite{Lin2016abruijin} and \emph{wtdbg} (\url{https://github.com/ruanjue/wtdbg2}) are two representatives of long-read assemblers that used a modified version of de Bruijin graph.
The first tool introduces the definition of ABruijin graphs that only take a subset of ``solid strings'' instead of all fixed length words decomposed from reads. The solid strings are chosen based on the \emph{k-mer} spectrum in which their correctnesses are confident. %%LC comment correctnesses is not a word!
Since the $k-1$ overlaps between consecutive \emph{k-mer}s are not presented all the time anymore,
%%LC what does that mean, is this really an overlap method ?
a fast dynamic programming algorithm is used to evaluate overlaps between read pairs based on their longest common subpaths. The draft assembly is constructed by adding vertices of those overlapped reads before undergoing a polishing step by aligning it to the original reads.
The tool wtdbg (now wtdbg2) on the other hand, builds a fuzzy de Bruijin graph for genome assembly. This graph is similar to the original DBG but allows mismatches and indels so that approximate \emph{k-mer}s can be collapsed into one node of the graph. Furthermore, the read paths are kept during decomposition and collapsing. After layout construction with fuzzy de Bruijin graph, a POA-like consensus step is invoked to generate the final assembly. The tool is reportedly able to quickly finish an assembly but normally consumes more memory %%LC comment than a normal DBG? 
for the new graph structure. 

For exclusive long reads assembly, the quality of assembly is in general lesser than NGS counterparts, even after a polishing step.  %LC have you described polishing ? 
...
Another approach is to utilize the erroneous long reads as the linker to connect the incomplete NGS assemblies~\cite{DeshpandeFP2013,BoetzerP2014}. Assemblers from this class usually require less long read coverage. Moreover, bridging operations are much less resource-consuming than aforementioned approaches.  I expand on these hybrid assemblers in the next section. 
...
Likewise, in term of assembling with long-read data, SSPACE-LongRead~\cite{BoetzerP2014} stands out as a widely-used software for this particular task. %%LC comment you should describe how the algorithm works
...
Although long reads from other sources, such as MinION, can also be used by these methods~\cite{Karlsson2015}, it usually requires some degree of parameter calibration. Moreover, since the quality and quantity of data is varied case by case, the application of these methods on novel datasets often returns less significant results than expected. %%LC comment not sure what is the point of this last sentence
...
Beside the genome scaffolders outlined above, there is a class of hybrid sequence assemblers that take advantage of error-free short reads to correct the nanopore reads at base level. These corrected long reads are then subjected to an assembler in the next step. Nanocorr~\cite{GoodwinGE2015} and NaS~\cite{MadouiEC2015} are two representatives for this approach. Normally, these assemblers provide high quality assemblies but are computationally expensive.  %%LC comment , didnt you discuss this approach in the previous section ? Also, do they really lead to better quality assemblies?
...
Its hybrid assembly module likewise works in batch mode to traverse the input assembly graph and return a polished assembly as the result. %%LC comment - what do you mean batch mode.  I think you should probably spend some more space to discuss unicycler
...
\paragraph{Goals setting} Initially, the project targets to implement an streaming pipeline to rapidly finish short-read assemblies using real-time MinION sequencing. 
...
On the other hand, it is desired to extend aforementioned pipeline for multiple samples at the same time, through a parallelization mechanism known as barcoded sequencing.
...
%Other than that, I developed utility tools not mentioned in details but will include one in chapter 4 for concatermers detection...
